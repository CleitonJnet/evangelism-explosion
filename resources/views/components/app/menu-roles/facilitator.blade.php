...
<flux:sidebar.group :heading="__('Plataforma do Facilitador')" :class="$sidebarGroupClass">


    @can('access-facilitator')
        <flux:sidebar.item :href="route('app.facilitator.dashboard')"
            :current="request()->routeIs('app.facilitator.dashboard')" wire:navigate :class="$sidebarItemClass">
            <div class="relative"><span class="text-lg">&#10174;</span> {{ __('Dashboard') }}
                <div style="text-shadow: 0 0 1px #000000"
                    class="absolute top-1/2 -translate-1/2 right-0 text-amber-200/90 {{ request()->routeIs('app.facilitator.dashboard') ? 'opacity-100' : 'opacity-0' }}">
                    &#10148;
                </div>
            </div>
        </flux:sidebar.item>
