...
<flux:sidebar.group :heading="__('Platform')" :class="$sidebarGroupClass">
    ...
    @can('access-mentor')
        ...
        <flux:sidebar.item :href="route('app.mentor.ojt.index')" :current="request()->routeIs('app.mentor.ojt.*')"
            wire:navigate :class="$sidebarItemClass">
            <div class="relative"><span class="text-lg">&#10174;</span> {{ __('OJT') }}
                <div style="text-shadow: 0 0 1px #000000"
                    class="absolute top-1/2 -translate-1/2 right-0 text-amber-200/90 {{ request()->routeIs('app.mentor.ojt.*') ? 'opacity-100' : 'opacity-0' }}">
                    &#10148;
                </div>
            </div>
        </flux:sidebar.item>
