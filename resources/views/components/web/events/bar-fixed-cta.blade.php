...
@php
    $hasValidDate = filled($date) && strtotime((string) $date) !== false;

    $isValidTime = static function (mixed $time): bool {
        if (!filled($time)) {
            return false;
        }

        $parsedTime = strtotime((string) $time);

        if ($parsedTime === false) {
            return false;
        }

        return !in_array(date('H:i:s', $parsedTime), ['00:00:00'], true);
    };

    $hasValidStartTime = $isValidTime($start_time);
    $hasValidEndTime = $isValidTime($end_time);
    $showCountdown = $hasValidDate && $hasValidStartTime && $hasValidEndTime;
@endphp
        ...
        <div class="items-center hidden min-w-0 gap-3 md:flex">
            ...
            <div class="min-w-0">
                ...
                <p class="text-xs truncate text-white/80">
                    ...
                    @elseif($hasValidDate)
                        Começa dia {{ date('d/m', strtotime($date)) }} • Horário a definir
                    @else
