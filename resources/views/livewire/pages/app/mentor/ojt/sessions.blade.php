                ...
                <tbody class="divide-y divide-[color:var(--ee-app-border)]">
                    @forelse ($sessions as $session)
                        <tr wire:key="mentor-ojt-session-{{ $session->id }}">
                            ...
                            <td class="px-4 py-3">
                                {{ $session->label ?: __('Sessão :number', ['number' => $session->sequence]) }}</td>
                            <td class="px-4 py-3">{{ $session->starts_at?->format('d/m/Y H:i') ?? __('A definir') }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-6 text-center text-sm text-[color:var(--ee-app-muted)]">
                                {{ __('Nenhuma sessão STP vinculada ao seu perfil.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
