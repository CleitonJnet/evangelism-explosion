<div class="space-y-6">
    <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="space-y-2">
                ...
                <h2 class="text-2xl font-bold text-slate-900">{{ trim(($training->course?->type ?? '').' '.($training->course?->name ?? '')) }}</h2>
                <div class="text-sm text-slate-600">{{ $training->church?->name ?? __('Igreja não informada') }}</div>
                <div class="text-sm text-slate-500">{{ $training->city ?: __('Cidade não informada') }}{{ $training->state ? ', '.$training->state : '' }}</div>
                <div class="text-sm text-slate-500">{{ __('Professor responsável') }}: {{ $training->teacher?->name ?? __('Não informado') }}</div>
            </div>

            <div class="flex flex-wrap gap-2">
                <flux:button variant="outline" :href="route('app.mentor.trainings.ojt', $training)">{{ __('Abrir STP/OJT') }}</flux:button>
                <flux:button variant="ghost" :href="route('app.mentor.ojt.sessions.index')">{{ __('Sessões') }}</flux:button>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('Sessões vinculadas') }}</div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('Participantes nas equipes') }}</div>
            <div class="mt-2 text-3xl font-bold text-slate-900">{{ $summary['students_count'] }}</div>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('Abordagens acompanhadas') }}</div>
