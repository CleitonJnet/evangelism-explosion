<div>
    <flux:modal name="teacher-inventory-stock-action-modal" wire:model="showModal"
                ...
                @if ($hasMaterials)
                    <div class="flex flex-wrap gap-x-4 gap-y-8">
                        <div style="flex: 1 0 420px">
                            <x-src.form.select name="teacher-inventory-stock-action-material" wire:model.live="material_id"
                                label="Material" width_basic="320" :options="$materialOptions" required />
                        </div>

                        @if ($mode === 'adjustment')
                            <x-src.form.input name="teacher-inventory-stock-action-target"
                                wire:model.live="target_quantity" :label="$modeMeta['quantity_label']" type="number"
                                width_basic="180" min="0" required />
                        @else
                            <x-src.form.input name="teacher-inventory-stock-action-quantity" wire:model.live="quantity"
                                :label="$modeMeta['quantity_label']" type="number" width_basic="180" min="1" required />
                        @endif

                        <x-src.form.textarea name="teacher-inventory-stock-action-notes" wire:model.live="notes"
                ...
                @else
                    <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5">
                        ...
                        <p class="mt-2 text-sm text-amber-900/90">
                            {{ __('Não é possível registrar movimentações enquanto este estoque não tiver itens transferidos ou compostos disponíveis pela composição atual.') }}
                        </p>
                    </div>
                @endif
            </div>
