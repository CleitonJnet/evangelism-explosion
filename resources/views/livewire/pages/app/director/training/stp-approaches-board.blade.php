...
<div>
    ...
    <x-src.toolbar.nav>
        ...
        <x-src.toolbar.button :href="route('app.director.training.statistics', $training)" :label="__('STP')" icon="users-chat" :tooltip="__('Saída de Treinamento Prático')" />
    </x-src.toolbar.nav>
    ...
    <div class="rounded-2xl bg-linear-to-br from-slate-100 via-white to-slate-200 p-4">
        <div class="mb-4 flex flex-wrap items-center gap-2">
            ...
            <x-src.btn-silver wire:click="createPlannedApproach('security_questionnaire')" :label="__('+ Questionário de Segurança')" />
        ...
        @else
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                @foreach ($teams as $team)
                    <section class="flex h-full flex-col rounded-xl border border-yellow-500 bg-yellow-100/20 p-3"
                        ...
                        <div class="js-stp-approach-list min-h-32 flex-1 space-y-2 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-2"
    ...
    <flux:modal name="stp-approach-report" wire:model="showModal" class="max-w-6xl w-[calc(100%-4px)] mx-auto bg-slate-100! p-0! max-h-[calc(100vh-4px)]! overflow-hidden">
        @if ($editingApproachId !== null && $editingApproach)
            <div class="relative flex max-h-[90vh] flex-col" x-data="{ showSavedMessage: false, savedMessage: '', hideTimer: null }"
                ...
                "
                x-on:focus-person-name.window="
                    $nextTick(() => {
                        const input = $refs.personNameField?.querySelector('input');
                        if (input) {
                            input.focus();
                        }
                    });
                ...
                <div class="sticky top-0 z-20 border-b border-sky-800 bg-sky-950 px-6 py-4 text-slate-100">
                    <flux:heading size="lg" class="!text-slate-100">{{ __('Formulário por Abordagem') }}: <span
                            class="font-bold uppercase">{{ $editingApproachTypeLabel }}</span>
                    </flux:heading>
                ...
                <div class="min-h-0 flex-1 space-y-4 overflow-y-auto bg-slate-100 px-6 py-4">
                    <div class="flex flex-wrap gap-4">
                        <div class="rounded-xl border border-slate-300 bg-white p-4 space-y-6">
                            ...
                            <div class="grid gap-5 md:grid-cols-2">
                                <div x-ref="personNameField">
                                    <x-src.form.input name="form.person_name" wire:model.live="form.person_name"
                                        label="Nome" width_basic="1000" />
                                </div>
                                ...
                                <div class="flex flex-wrap gap-3 hover:bg-slate-50 transition pb-2 pt-8"
                                    ...
                                    <x-src.form.select name="secound_question"
                                        ...
                                        label="Resultado" width_basic="200" :options="[
                                            ...
                                            [
                                                'value' => 'no_decision_interested',
                                                'label' => 'Sem decisão / interessado',
                                            ],
                        ...
                        <div class="rounded-xl border border-slate-300 bg-white/70 p-4 space-y-3">
                            ...
                            <div class="grid gap-6 md:grid-cols-2">
                                ...
                                <div class="md:col-span-2">
                                    <x-src.form.textarea name="form.payload.notes"
                                        wire:model.live="form.payload.notes" label="Observações da abordagem"
                                        width_basic="1000" />
                                </div>
                            </div>
                ...
                <div class="sticky bottom-0 z-20 border border-sky-800 bg-sky-950 px-6 py-4">
                    <div class="flex justify-between gap-3">
                        <div class="flex gap-2">
                            @if ($this->canDeleteApproach())
                                <button type="button"
                                    class="inline-flex items-center justify-center rounded-xl border border-red-300 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700 transition hover:bg-red-100"
                ...
                <div class="pointer-events-none absolute inset-x-0 bottom-24 z-30">
