...
<div x-data="{ showRegenerateScheduleModal: false, showScheduleAttentionModal: @js($showScheduleAttentionModal) }" x-on:schedule-alert.window="window.alert($event.detail.message)"
    ...
    <section class="grid gap-6">
        @forelse ($eventDates as $eventDate)
            @php
                ...
                $planStatus =
                    $planStatusByDate[$dateKey] ?? \App\Livewire\Pages\App\Director\Training\Schedule::PLAN_STATUS_UNDER;
                $dayBorderClass =
                    $planStatus === \App\Livewire\Pages\App\Director\Training\Schedule::PLAN_STATUS_OVER
                        ? 'border-2 border-red-400'
                        : ($planStatus === \App\Livewire\Pages\App\Director\Training\Schedule::PLAN_STATUS_OK
            ...
            <div id="{{ 'day' . $loop->iteration }}"
                ...
                @endphp
                @if ($planStatus === \App\Livewire\Pages\App\Director\Training\Schedule::PLAN_STATUS_UNDER)
                ...
                @elseif ($planStatus === \App\Livewire\Pages\App\Director\Training\Schedule::PLAN_STATUS_OVER)
    ...
    <livewire:pages.app.director.training.edit-event-dates-modal :trainingId="$training->id"
        wire:key="edit-event-dates-modal-{{ $training->id }}" />
    <livewire:pages.app.director.training.edit-event-banner-modal :trainingId="$training->id"
