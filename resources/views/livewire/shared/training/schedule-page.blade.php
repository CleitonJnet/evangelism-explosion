...
<div x-data="{ showRegenerateScheduleModal: false, showScheduleAttentionModal: @js($showScheduleAttentionModal) }" x-on:schedule-alert.window="window.alert($event.detail.message)"
    ...
    <section class="grid gap-6">
        @forelse ($eventDates as $eventDate)
            @php
                ...
                $planStatus = $planStatusByDate[$dateKey] ?? 'under';
                $dayBorderClass = $planStatus === 'over'
                    ? 'border-2 border-red-400'
                    : ($planStatus === 'ok' ? 'border border-emerald-500' : 'border-2 border-amber-400');
    ...
    @livewire($this->contextComponent('editEventDatesModal'), ['trainingId' => $training->id], key('edit-event-dates-modal-'.$training->id.'-'.$this->contextRoute('show')))
    @livewire($this->contextComponent('editEventBannerModal'), ['trainingId' => $training->id], key('edit-event-banner-modal-'.$training->id.'-'.$this->contextRoute('show')))
