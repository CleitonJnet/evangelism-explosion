<x-layouts.app :title="__('OJT Sessions')">
    <x-src.toolbar.bar :title="__('On-The-Job Training')" :description="__('Manage OJT sessions for this training.')">
        ...
        <x-src.toolbar.button :href="route('app.teacher.training.show', $training)" :label="__('Details')" icon="eye" :tooltip="__('Training details')" />
    ...
    <div class="mt-4 flex flex-wrap items-center justify-center gap-2 text-sm">
        <x-src.toolbar.button :href="route('app.teacher.trainings.ojt.sessions.index', $training)" :label="__('Sessions')" icon="calendar-check" :active="true" />
    ...
    <div class="mt-6">
        <livewire:pages.app.teacher.training.ojt.sessions :training="$training" />
    </div>
</x-layouts.app>
